...
for file in *.md.html; do
    if [[ -f "$file" ]]; then
        ...
        if grep -q "<!DOCTYPE html>" "$file" && grep -q "journal.css" "$file"; then
        ...
        # Extract content between <code> tags, excluding the wrapper
        content=$(sed -n '/<code style="display:none">/,/<\/code>/p' "$file" | sed '1d;$d')
        
        if [ -z "$content" ]; then
            echo "⚠️  No content found in $file, skipping..."
            continue
        fi
