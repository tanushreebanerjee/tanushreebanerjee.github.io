...
[Figure placeholder: Visualization showing unordered image collection → feature matching → camera pose estimation → sparse 3D point cloud]
...
**Insight**: SfM produces sparse reconstruction, then MVS makes it dense. This two-stage pipeline (sparse → dense) is common pattern.
