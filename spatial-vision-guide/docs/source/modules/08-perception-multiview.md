...
```
Multi-view Images → Image Encoder → 2D Features
                                            ↓
3D Object Queries ←→ [Project to 2D] ←→ Sample Features
      ↓
Refine 3D Boxes
```
...
- **DETR**: 2D object queries → 2D features → 2D boxes
- **DETR3D**: 3D object queries → (project to) 2D features → 3D boxes
...
- **BEV**: 2D features → BEV features → 3D detection
- **Multi-view transformers**: 2D features → (3D-aware) → Direct 3D detection
...
2. **3D position encoding**: Camera calibration → unproject → positional encoding
...
[Figure placeholder: Comparison showing traditional pipeline (images → SfM → 3D → detection) vs multi-view transformer (images → transformer → 3D detection directly)]
