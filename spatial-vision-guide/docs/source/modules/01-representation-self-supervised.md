...
**Augmentation policy**: Stronger augmentations → better generalization.
...
[Figure placeholder: Comparison showing supervised learning (labeled images → expensive) vs self-supervised (unlabeled images → free)]
...
**Broader impact**: Shows that task design matters. Better pretext tasks → better features. Encourages exploration of novel pretext tasks.
...
[Figure placeholder: Applications diagram showing self-supervised pre-training → downstream tasks: classification, detection, segmentation, retrieval, etc.]
