...
**BEV grid**: Regular 2D grid in bird's-eye view (e.g., 200m × 200m, 0.5m resolution).
...
**Grid structure**: Regular grid (e.g., 400×400 cells, each 0.5m×0.5m).
...
**Approaches**:
- **LSS**: Depth distribution → frustum of 3D points
...
**Insight**: BEV demonstrates value of explicit intermediate representations. Rather than direct 2D→3D mapping, intermediate BEV representation enables:
...
**BEV methods**:
- 2D features → BEV features → 3D detection
...
- 2D features → Direct 3D detection
