...
**Projection**: 3D Gaussian → 2D Gaussian in image space.
...
**Split**: Large Gaussians (high gradient) → split into smaller ones.

**Clone**: Small Gaussians with high opacity → duplicate in neighboring views.
...
3. **Loss**:
   ...
   - $L_1$: Pixel-wise L1 loss: $L_1 = \frac{1}{N} \sum_{i=1}^N |I_i - \hat{I}_i|$
   - $L_{D-SSIM}$: Structural similarity loss (perceptual loss)
   - Typical $\lambda = 0.2$ (more weight on L1, some on SSIM)

[Figure placeholder: Loss function visualization:
- Left: Rendered image
- Center: Ground truth image
- Right: Loss map (red = high error, blue = low error)
- Shows L1 loss emphasizes pixel differences, SSIM emphasizes structure]

4. **Optimization**:
   - SGD with exponential decay: $\eta_t = \eta_0 \cdot \gamma^t$
   - Learning rate: $\eta_0 = 0.01$, decay $\gamma = 0.99$
   - Adaptive density control every $N$ iterations (typically every 100 iterations)

5. **Convergence**: Typically 7,000-30,000 iterations.

**Code example** (Gaussian rasterization):
```python
def rasterize_gaussians(gaussians, camera):
    """
    Rasterize 3D Gaussians to image.

    Args:
        gaussians: List of Gaussian parameters (μ, Σ, opacity, color)
        camera: Camera parameters (intrinsics, extrinsics)
    ...
    for gaussian in gaussians:
        # Project to 2D
        mu_2d = project_3d_to_2d(gaussian.mu, camera)
        cov_2d = project_covariance_3d_to_2d(gaussian.cov, camera)
        ...
        for y in range(H):
            for x in range(W):
                ...
                diff = pixel - mu_2d
                exponent = -0.5 * diff.T @ torch.inverse(cov_2d) @ diff
                alpha = gaussian.opacity * torch.exp(exponent)

                # Alpha blending
                image[y, x] = alpha * gaussian.color + (1 - alpha) * image[y, x]

    return image
```

**Note**: Actual implementation uses efficient tile-based rasterization, not pixel-by-pixel.
...
**Problem**: Requires high-quality point cloud from COLMAP/SfM for initialization. Poor initialization → poor results.
...
**Problem**: Large scenes require millions of Gaussians → large memory footprint (hundreds of MB to GB).
...
**Broader impact**: Highlights value of multi-stage pipelines (SfM → Gaussian Splatting). Classical methods still provide valuable preprocessing for deep learning.
